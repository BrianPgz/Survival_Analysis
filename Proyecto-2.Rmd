...
```{r Modelos individuales, include=FALSE}
modelo_cox = coxph(Surv(N_Days, delta) ~ Sex + factor(Stage) + Hepatomegaly + factor(Edema) +factor(Drug), base_nueva)
```
...
```{r}
prueba= ( 0.4977-.1  )/(0.2383) 
pnorm(q=prueba, lower.tail = FALSE  )
```
